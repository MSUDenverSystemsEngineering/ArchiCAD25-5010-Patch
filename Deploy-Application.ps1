...
Try {
	...
	[string]$appScriptDate = '04/15/2022'
	...
	If ($deploymentType -ine 'Uninstall' -and $deploymentType -ine 'Repair') {
		...
		$installStatus = Get-InstalledApplication -Name 'ArchiCAD'
		If ($installStatus){
			## Update the License
			If (Test-Path -Path "C:\ProgramData\ARCHICAD\AC_25_USA\education.bak") {
				Write-Log -Message "License archive already exists. Deleting existing file..." -Source 'Pre-Installation' -LogType 'CMTrace'
				Remove-Item -Path "C:\ProgramData\ARCHICAD\AC_25_USA\education.bak" -Force
			}
			If (Test-Path -Path "C:\ProgramData\ARCHICAD\AC_25_USA\") {
				Write-Log -Message "License Path Exists" -Source 'Pre-Installation' -LogType 'CMTrace'
				If (Test-Path -Path "C:\ProgramData\ARCHICAD\AC_25_USA\education.lic") {
					Write-Log -Message "License file already exists." -Source 'Pre-Installation' -LogType 'CMTrace'
					Write-Log -Message "Archiving the old license file..." -Source 'Pre-Installation' -LogType 'CMTrace'
					Rename-Item -Path "C:\ProgramData\ARCHICAD\AC_25_USA\education.lic" -NewName "education.bak"	## We'll use this as a detection method for the deployment so we can make sure the old license was replaced
				...
				$licenseFile = Test-Path -Path "C:\ProgramData\ARCHICAD\AC_25_USA\education.lic"	## Should evaluate false
				If (!$licenseFile) {
					Write-Log -Message "Importing the new license file..." -Source 'Pre-Installation' -LogType 'CMTrace'
			...
			Else {
				Write-Log -Message "Path not found: C:\ProgramData\ARCHICAD\AC_25_USA\" -Source 'Pre-Installation' -LogType 'CMTrace'
				Write-Log -Message "Creating directory: C:\ProgramData\ARCHICAD\AC_25_USA\" -Source 'Pre-Installation' -LogType 'CMTrace'
				New-Item -Path "C:\ProgramData\ARCHICAD\AC_25_USA" -ItemType "directory"
				Write-Log -Message "Importing the new license file..." -Source 'Pre-Installation' -LogType 'CMTrace'
				Copy-Item "$dirSupportFiles\education.lic" -Destination "C:\ProgramData\ARCHICAD\AC_25_USA"
				If (Test-Path -Path "C:\ProgramData\ARCHICAD\AC_25_USA\education.lic"){
					Write-Log -Message "Import successful" -Source 'Pre-Installation' -LogType 'CMTrace'
